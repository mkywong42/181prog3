...
cout<<"got next slot"<<endl;
    ...
    if(highKey == NULL){
        ...
        if(compare(attribute, current, highKey)==0 && highKeyInclusive==false){
            ...
            if(ix_ScanIterator.currentEntry < 0){
                ...
                ix_ScanIterator.currentEntry=prevNodeHeader.indexEntryNumber-1;
                ix_ScanIterator.currentPage = nodeHeader.leftPageNum;
    ...
    // ixScanIterator.lowkey = lowKey;
    // ixScanIterator.highkey = highKey;
    ix_ScanIterator.includeLow = lowKeyInclusive;
    ix_ScanIterator.includeHigh = highKeyInclusive;
    return SUCCESS;
}
